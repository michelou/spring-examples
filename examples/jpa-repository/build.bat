...
@rem                    _JAVA_CMD, _JAVAC_CMD, _MVN_CMD
:env
set _BASENAME=%~n0
set "_ROOT_DIR=%~dp0"
set _TIMER=0
...
if "%__ARG:~0,1%"=="-" (
    ...
    ) else if "%__ARG%"=="run" ( set _COMMANDS=!_COMMANDS! run
...
rmdir /s /q "!__DIR!"
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to delete directory "!__DIR:%_ROOT_DIR%=!"
    set _EXITCODE=1
    goto :eof
)
goto :eof
...
:run
set __MVN_OPTS=
if %_DEBUG%==1 ( echo %_DEBUG_LABEL% "%_MVN_CMD%" %__MVN_OPTS% spring-boot:run 1>&2
) else if %_VERBOSE%==1 ( echo Start Spring Boot application 1>&2
)
call "%_MVN_CMD%" %__MVN_OPTS% spring-boot:run
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to start Spring Boot application 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
