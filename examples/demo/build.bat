...
) else if %_VERBOSE%==1 ( echo Start Spring boot server "%_SERVER_PROC_NAME%" 1>&2
)
start "%_SERVER_PROC_NAME%" "%_MVN_CMD%" %__MVN_OPTS% spring-boot:run
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to start Spring boot server "%_SERVER_PROC_NAME%" 1>&2
    set _EXITCODE=1
    goto :eof
)
...
:run_browser
set __URL=http://localhost:8080/hello
...
set __LAUNCHER_ARGS="--classpath=%_TEST_CLASSES_DIR%" "--select-package=com.example.demo"
